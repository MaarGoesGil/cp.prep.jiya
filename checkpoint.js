...
function stringMasLarga(str) {
  // La función llamada 'stringMasLarga', recibe como argumento una frase (string) 'str'
  ...
  // Tu código aca:
  let palabra = str.replace(",", " ").split(" ") ;
  let palabraLarga = "" ;
  palabra.filter ( (Palabra) => {
...
function estaOffline(usuarios, nombre) {
  // La funcion llamada "estaOffline" recibe como argumento un array de objetos llamado 'usuarios' y un string llamado 'nombre'.
...
function buscaDestruye(arreglo, num) {
  ...
  // La función debe retornar el array con los números sacados.
...
function sumarElTipo(arreglo) {
  ...
  // Tu código aca:
  let objt = arreglo.reduce((contador,vehiculo) => {
...
function crearClaseEmprendedor() {
  class Emprendedor {
      ...
      getBooks() {
          ...
          { arregloNew.push (this.libros[i]['nombre']) }
          return arregloNew ;
      }
